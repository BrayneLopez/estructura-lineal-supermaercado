        ...
        try:
            ...
            if len(str(lector_code)) == 8 or lector_code in [0, 1, 2]: 
                if  any(lector_code == confirmation for confirmation in state_app['option-numbers']['RESPONSE_BOOL_OPTION']):
                    state_app['logs-generals']['logs-error-buy']['logs-process'].setdefault(time_procesin, state_app['types_error_code'][state_app['option-numbers']['REQUEST_NUMBER_ERROR'][lector_code]])
        ...
        for valitycoding in state_app['sesion-modulos']['code-safety']: # RECOORER LOS CODIGOS INGRESADOS
            if valitycoding in state_app['logs-generals']['logs-code-insert-bd']: # CONSULTA SI ESTA EN LA BD DE CODIGOS
                ...
                if valitycoding in state_app['logs-generals']['logs-delete-buy']['logs-coding-cache']: # ELSE > BUSCA SI ESTA EN LOS CODIGOS YA FACTURADOS
                    ...
                    return False, state_app['logs-generals']['logs-error-buy']['logs-product'].setdefault(time_procesin, state_app['types_error_code'][555]) # MUESTRA CODE 555 > FACTURADO
                else:
                    return False, state_app['logs-generals']['logs-error-buy']['logs-product'].setdefault(time_procesin, state_app['types_error_code'][222]) # ELSE> PRODUCTO NO ESTA EN BD - CACHE > NO EXISTE
        return True, None
    return False, None
    ...
    if sessions_payment:
        try:
            ...
            if process_buy_tarjet == 0:
                state_app['logs-generals']['logs-error-buy']['logs-process']
...
def logs_delete_product(update_values, state_app, message_on):
    if update_values:
        ...
        if message_on in types_error_code.values():
            print(error_code_view[message_on])
            return error_code_view[message_on]
           
           
def memory_canast_clear(memoryclean, state_app):
    if memoryclean:
...
remove_products = sales_registers(immpresion_ticket, usuario='XXXXXXXXXX', fecha=time, cupon=True, canasta=state_app['sesion-modulos'].get('canasta'), precio=state_app['sesion-modulos'].get('total'))

update_values = update_almacen(remove_products, state_app)

memoryclean = logs_delete_product(update_values, state_app, message_on)

memory_canast_clear(memoryclean, state_app)
